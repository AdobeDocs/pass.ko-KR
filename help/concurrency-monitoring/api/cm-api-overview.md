---
title: API 사용 예
description: 동시성 모니터링의 API 끝점 사용
...
자세한 내용은 [온라인 API 설명서](https://streams-stage.adobeprimetime.com/swagger-ui/index.html)를 참조하세요.

## 목적 및 사전 요구 사항 {#purpose-prerequisites}

이 문서는 Concurrency Monitoring과의 통합을 구현할 때 애플리케이션 개발자가 Swagger API 사양을 사용하는 데 도움이 됩니다. 이 지침을 따르기 전에 독자가 서비스에서 정의한 개념에 대해 이전에 이해한 적이 있는 것이 좋습니다. 이를 이해하려면 [제품 설명서](../cm-home.md) 및 [Swagger API 사양](https://streams-stage.adobeprimetime.com/swagger-ui/index.html)에 대한 개요가 필요합니다.
...
응답 본문 필드에서 볼 수 있듯이 메타데이터 속성 목록이 비어 있습니다. 즉, 설계에 필요한 속성이 이 애플리케이션에 할당된 3 스트림 정책을 평가하기에 충분하다는 의미입니다. [표준 메타데이터 필드 설명서](../technical/standard-metadata-attributes.md)를 참조하세요. 이 호출 후에 계속 진행하여 세션 REST 리소스에 새 세션을 만들 수 있습니다.
