...
MVPD M에는 iPhone 애플리케이션, iPad 애플리케이션 및 웹 사이트가 있습니다. 프로그래머 P의 요청에 따라 Adobe Concurrency Monitoring(CM)과 통합해야 합니다. 프로그래머 P는 CM에서 동시 사용을 제한하는 규칙이 있는 정책 집합을 정의했다. CM은 정의된 정책 및 규칙을 기반으로 어떤 스트림의 재생이 허용되는지에 대한 결정을 내립니다. 즉, 스트림을 시작 및 재생할 수 있더라도 재생 중에 CM이 중지를 결정할 수 있습니다.
...
| 하트비트 간격 | Adobe 동시 실행 모니터링으로 하트비트 호출을 전송하는 간격(초) | 60 | 60 |
| 스트림 준수 간격 | Adobe 동시성 모니터링에 대한 스트림 호환성을 확인하는 간격(초) | 180 | 180 |
