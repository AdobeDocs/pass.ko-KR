---
title: 동시 모니터링에서 VOD 및 라이브 컨텐츠를 구분하는 방법
description: 동시 모니터링에서 VOD 및 라이브 컨텐츠를 구분하는 방법
...
# 방법: 동시 모니터링에서 VOD 및 라이브 컨텐츠 구분 {#dist-vod-live}

**Q:** 동시성 모니터링 서비스에서 재생 중인 콘텐츠 유형(라이브 콘텐츠와 Video on demand)을 구분할 수 있습니까?



**A:** 동시성 모니터링에서 라이브 콘텐츠와 주문형 비디오(VOD)를 직접 구분할 수 없습니다. 비디오 플레이어가 재생 중인 콘텐츠 형식을 알고 있어야 하며 [세션 초기화 호출](/help/concurrency-monitoring/api/cm-api-overview.md#session-initial) 중에 이 정보를 보내야 합니다(동시 모니터링에 필요). 일반 워크플로우는 다음과 같습니다.
...
* [동시 모니터링 표준 메타데이터 속성](/help/concurrency-monitoring/technical/standard-metadata-attributes.md)
* [동시성 모니터링 API 개요](/help/concurrency-monitoring/api/cm-api-overview.md)
