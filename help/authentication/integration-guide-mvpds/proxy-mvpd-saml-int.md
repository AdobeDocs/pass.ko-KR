...
이 문서에서는 프록시 통합을 위한 SAML 인증 흐름에 대해 설명합니다.  이러한 흐름은 Adobe Pass 인증 서버 구성에 있는 프록시 구성 데이터에 따라 다릅니다. 프록시 MVPD는 Adobe Pass 인증 프록시 웹 서비스를 통해 프록시 구성 데이터를 Adobe Pass 인증 서버에 푸시합니다.
...
| 속성 | 설명 |
|------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| MVPD ID | Adobe Pass 인증에 내부적으로 프록시화된 MVPD를 나타내는 문자열입니다.  Adobe Pass 인증 컨텍스트에서 고유한 것으로 Adobe에서 확인해야 합니다. |
...
MVPD 구독자가 프로그래머의 사이트 또는 애플리케이션을 방문할 때 Adobe Pass 인증은 해당 프로그래머에 대해 활성화된 MVPD 목록으로 사이트 또는 애플리케이션으로부터의 API 호출에 응답합니다.  통합은 직접적이거나 프록싱될 수 있으며, 프로그래머에게는 이들 간에 구별이 없다. 이를 통해 프로그래머는 자신이 적합하다고 보는 모든 방식으로 활성 MVPD 목록을 표시할 수 있습니다. 구독자는 MVPD를 선택하고 Adobe Pass 인증은 구독자를 MVPD의 특정 ID 공급자로 리디렉션합니다.

통합 MVPD 프록시의 경우 Adobe Pass 인증과 MVPD 프록시 간에 통합이 수행됩니다. Adobe Pass 인증은 사용자 인증 요청을 MVPD 프록시로 보내고 MVPD 프록시가 리디렉션을 처리합니다. MVPD 프록시가 사용자 인증 요청을 리디렉션할 위치를 알 수 있도록 Adobe Pass 인증은 SAML 인증 요청에서 MVPD 식별자를 보냅니다.  이 식별자는 위에 지정된 프록시 웹 서비스를 통해 프록시 공급자가 지정한 MVPD ID입니다.

### 인증 {#authn-saml-int}

Adobe Pass 인증을 프록시 MVPD와 통합하려면 다음 조건을 충족해야 합니다.
...
**범위 지정 속성** - 특정 MVPD_ID 및 MVPD 이름을 포함하는 IDPEntry 항목을 포함합니다.  사용자가 프로그래머 선택기에서 실제로 선택한 MVPD를 나타내며 프록시 웹 서비스에 지정된 MVPD_ID와 일치합니다.
