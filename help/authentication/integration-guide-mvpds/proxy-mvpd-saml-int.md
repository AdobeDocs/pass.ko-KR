...
이 문서에서는 프록시 통합을 위한 SAML 인증 흐름에 대해 설명합니다.  이러한 흐름은 Adobe Pass 인증 서버 구성에 있는 프록시 구성 데이터에 따라 다릅니다. 프록시 MVPD은 Adobe Pass 인증 프록시 웹 서비스를 통해 프록시 구성 데이터를 Adobe Pass 인증 서버에 푸시합니다.

## 프록시 구성 데이터 {#proxy-config-data}

각 MVPD 프록시는 프록시화된 MVPD에 대한 프록시 구성 데이터를 Adobe Pass 인증 프록시 웹 서비스에 제공합니다.  자세한 내용은 프록시 웹 서비스 설명서에 나와 있습니다.   SAML AuthN 흐름이 작동하려면 프록시 구성 데이터에 다음 속성이 포함되어야 합니다.

| 속성 | 설명 |
|------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| MVPD ID | Adobe Pass 인증에 내부적으로 프록시화된 MVPD을 나타내는 문자열입니다.  Adobe Pass 인증 컨텍스트에서 Adobe이 고유한 것으로 확인합니다. |
| MVPD 기본 로고 URL | 사용자의 MVPD 선택기 환경에 표시할 수 있는 로고 URL입니다.  투명 배경을 사용해야 합니다. |
...
MVPD 구독자가 프로그래머의 사이트 또는 애플리케이션을 방문하면 Adobe Pass 인증은 해당 프로그래머에 대해 활성화된 MVPD 목록으로 사이트 또는 애플리케이션으로부터의 API 호출에 응답합니다.  통합은 직접적이거나 프록싱될 수 있으며, 프로그래머에게는 이들 간에 구별이 없다. 이를 통해 프로그래머는 자신이 적합하다고 보는 모든 방식으로 활성 MVPD 목록을 표시할 수 있습니다. 구독자가 MVPD을 선택하면 Adobe Pass 인증이 구독자를 MVPD의 특정 ID 공급자로 리디렉션합니다.

통합 MVPD 프록시의 경우 통합은 Adobe Pass 인증과 MVPD 프록시 간에 수행됩니다. Adobe Pass 인증은 MVPD 프록시로 사용자 인증 요청을 보내고 MVPD 프록시가 리디렉션을 처리합니다. MVPD 프록시가 사용자 인증 요청을 리디렉션할 위치를 알 수 있도록 Adobe Pass 인증은 SAML 인증 요청에서 MVPD 식별자를 보냅니다.  이 식별자는 위에 지정된 프록시 웹 서비스를 통해 프록시 공급자가 지정한 MVPD ID입니다.

### 인증 {#authn-saml-int}

Adobe Pass 인증을 프록시 MVPD과 통합하려면 다음 조건을 충족해야 합니다.

* 프록시 MVPD에서 프록시 처리된 MVPD 목록을 제공하고 Adobe 프록시 웹 서비스에 푸시함

* 상위 MVPD 프록시에 대한 SAML 메타데이터

* (권장) - 프록시 MVPD은 프록시화된 MVPD의 로그인 페이지 URL로 추가 리디렉션을 처리합니다
...
SAML AuthN 요청에서 프록시 통합에는 MVPD 프록시에서 처리해야 하는 다음 추가 속성이 포함됩니다.  이 속성은 프록시화된 MVPD을 대신하여 요청자를 올바르게 처리하고 올바른 로그인 경험을 렌더링하기 위해 필요합니다. (이 속성은 아래 샘플 요청에서 강조 표시됩니다.)

**범위 지정 속성** - 특정 MVPD_ID 및 MVPD 이름을 포함하는 IDPEntry 항목을 포함합니다.  사용자가 프로그래머 선택기에서 실제로 선택한 MVPD을 나타내고 프록시 웹 서비스에 지정된 MVPD_ID와 일치합니다.

필요한 경우 프로그래머의 특정 브랜드에 대한 로그인을 사용자 지정하는 데 사용할 수 있는 RequestorID에 대한 추가 범위 속성이 있습니다. 또는 요청이 시작되는 위치의 분석에 간단히 사용할 수 있습니다.

SAML AuthN 응답에서 프록시 MVPD은 다음 속성에서 프록시화된 MVPD을 IdP 엔티티로 지정해야 합니다.
...
인증 부분의 경우 MVPD은 프로그래머가 지정한 리소스의 인증을 수락해야 합니다.  대부분의 경우, 이는 TBS 또는 TNT와 같은 채널 네트워크에 대한 문자열 식별자이다.
