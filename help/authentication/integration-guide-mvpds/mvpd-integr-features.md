...
| 아니. | 기능 | 설명 | 우선순위 | 메모 |
|------|---------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|----------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1.1 | [프로그래머가 시작한 로그인](/help/authentication/integration-guide-mvpds/authn-oauth2-protocol.md) | 뷰어는 MVPD을 선택하고 프로그래머 브랜드의 웹 사이트 또는 애플리케이션에서 인증(AuthN) 흐름을 시작합니다. | A+ |                                                                                                                                                          |
| 1.2 | [채널 기반 인증](/help/authentication/integration-guide-mvpds/authz-usecase.md) | 인증되면 백그라운드에서 네트워크 채널 식별자와 사용자 식별자만 MVPD으로 전달하여 인증(AuthZ)을 수행할 수 있습니다. | A+ |                                                                                                                                                          |
| 1.3 | 사용자 ID 지속성 | MVPD은 난독화되고 영구적인 사용자 ID를 제공합니다. | A |                                                                                                                                                          |
| 1.4 | [Single Sign-On 지원](/help/authentication/integration-guide-programmers/legacy/sso-access/sso-support.md) | 뷰어는 한 브랜드의 사이트에서 MVPD으로 로그인하므로 다른 브랜드로 이동하여 다시 로그인하라는 메시지가 표시되지 않을 수 있습니다. AuthN 세션은 브랜드 간에 공유됩니다. SSO 지원은 웹 사이트와 모바일/디바이스 애플리케이션 모두에 해당됩니다.  MVPD의 경우, 사용자 ID나 브랜드 간 AuthZ에 사용할 수 있는 다른 사용자 토큰을 반환해야 합니다. | A |                                                                                                                                                          |
| 1.5 | 장치에 최적화된 UX(로그인 사용자 경험) | MVPD에서는 보기가 사용 중인 디바이스의 크기에 맞게 로그인 화면 크기를 조정할 수 있습니다. | A |                                                                                                                                                          |
| 1.6 | 기본 MVPD 선택기 로고 | MVPD은 적절한 크기(112x33픽셀)의 기본 로고에 대한 URL을 제공합니다. | A | 로고는 MVPD에 의해 호스팅되어야 하며 CDN에 의해 캐시되어야 합니다. |
| 1.7 | [서비스 공급자 범위 지정](/help/authentication/integration-guide-mvpds/serv-provider-scoping.md) | MVPD은 AuthN 요청에서 브랜드 식별자(요청자 값) 전달을 지원합니다. | A- | 이렇게 하면 서비스 공급자별 로그인 경험이 활성화됩니다. |
| 1.8 | [다중 채널 인증](/help/authentication/integration-guide-mvpds/mvpd-preflight-authz.md#preflight-multich-authz) | MVPD은 프로그래머가 단일 인증 요청에서 여러 채널을 전송하는 것을 지원합니다. | B+ |                                                                                                                                                          |
| 1.9 | iFrame 또는 &quot;JS 팝업&quot; 기반 인증 | 프로그래머는 HTTP 리디렉션 대신 iFrame 또는 팝업 경험에 로그인 흐름을 통합할 수 있습니다. | B |                                                                                                                                                          |
| 1.10 | [프로그래머 시작 로그아웃](/help/authentication/integration-guide-mvpds/usecase-mvpd-logout.md) | 뷰어에는 프로그래머 사이트 또는 앱과 MVPD 모두에서 인증된 세션이 있습니다. 뷰어는 프로그래머 사이트에서 페더레이션 로그아웃을 시작할 수 있으며 로그아웃은 MVPD 포털의 세션도 지웁니다. | B | 프로그래머 관점에서 공유 컴퓨터가 오용되지 않도록 보호합니다. |
| 1.11 | [사용자 지정 인증 오류 메시지](/help/authentication/integration-guide-programmers/legacy/error-reporting/error-reporting.md) | MVPD은 사용자에게 표시할 프로그래머의 페더레이션 사이트 또는 앱에 적합한 자체 사용자 지정 오류 문자열을 전달합니다. | B | 상향 판매 시나리오 활성화 |
| 1.12 | **인증 응답에 대한 사용자 메타데이터** | MVPD AuthN 응답에는 자격 흐름 동안 사용자 경험을 개인화하기 위한 힌트로 사용되는 사용자 메타데이터가 포함될 수 있습니다. 이 요구 사항을 통해 MVPD에서 프로그래머로 부모 제어 힌트를 사용할 수 있습니다. | B- |                                                                                                                                                          |
| 1.13 | MVPD에서 시작한 인증 | 뷰어가 MVPD 포털에서 성공적인 AuthN 세션을 완료한 다음 프로그래머의 TVE 사이트로 이동합니다. MVPD 선택기를 묻는 메시지가 표시되지 않고 자동으로 인증됩니다. | B- |                                                                                                                                                          |
