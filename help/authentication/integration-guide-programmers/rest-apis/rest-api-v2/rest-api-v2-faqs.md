...
구성 단계의 목적은 구성 세부 사항(예: `id`, `displayName`, `logoUrl` 등)과 함께 현재 통합되는 MVPD 목록을 클라이언트 응용 프로그램에 제공하는 것입니다. 각 MVPD에 대해 Adobe Pass 인증으로 저장됨.

구성 단계는 클라이언트 애플리케이션이 사용자에게 TV 공급자를 선택하라고 요청해야 할 때 인증 단계에 대한 필수 단계 역할을 합니다.

#### &#x200B;2. 구성 단계는 필수입니다? {#configuration-phase-faq2}
...
#### &#x200B;3. 구성은 무엇이며 유효기간은 얼마나 됩니까? {#configuration-phase-faq3}
...
#### &#x200B;4. 구성이 서비스 공급자, 플랫폼 또는 사용자에게만 해당됩니까? {#configuration-phase-faq4}
...
#### &#x200B;5. 클라이언트 애플리케이션이 영구 스토리지에 구성 응답 정보를 캐시해야 합니까? {#configuration-phase-faq5}
...
#### &#x200B;6. 클라이언트 애플리케이션은 자체 MVPD 목록을 관리할 수 있습니까? {#configuration-phase-faq6}
...
#### &#x200B;7. 클라이언트 애플리케이션은 MVPD 목록을 필터링할 수 있습니까? {#configuration-phase-faq7}
...
#### &#x200B;4. 인증 코드란 무엇이며 얼마나 오랫동안 유효합니까? {#authentication-phase-faq4}
...
#### &#x200B;6. 사용자가 이미 인증되었는지 클라이언트 애플리케이션은 어떻게 알 수 있습니까? {#authentication-phase-faq6}
...
| 속성 | 사용자 경험 |
|--------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `mvpd` | 클라이언트 응용 프로그램은 이 옵션을 사용하여 선택한 TV 공급자를 추적하고 사전 승인 또는 승인 단계 동안 계속 사용할 수 있습니다.<br/><br/>현재 사용자 프로필이 만료되면 클라이언트 응용 프로그램에서 기억된 MVPD 선택을 사용하여 사용자에게 확인하도록 요청할 수 있습니다. |
| `attributes` | 클라이언트 애플리케이션은 이를 사용하여 다양한 [사용자 메타데이터](/help/authentication/integration-guide-programmers/features-standard/entitlements/user-metadata.md) 키(예: `zip`, `maxRating` 등)를 기반으로 사용자 경험을 개인화할 수 있습니다.<br/><br/>사용자 메타데이터는 인증 흐름이 완료된 후에 사용할 수 있으므로 [사용자 메타데이터](/help/authentication/integration-guide-programmers/features-standard/entitlements/user-metadata.md) 정보가 이미 프로필 정보에 포함되어 있으므로 클라이언트 애플리케이션은 별도의 끝점을 쿼리하지 않아도 됩니다.<br/><br/>특정 메타데이터 특성은 MVPD 및 특정 메타데이터 특성에 따라 인증 흐름 중에 업데이트될 수 있습니다. 따라서 클라이언트 애플리케이션은 최신 사용자 메타데이터를 검색하기 위해 프로필 API를 다시 쿼리해야 할 수 있습니다. |
| `notAfter` | 클라이언트 응용 프로그램은 이를 사용하여 사용자 프로필 만료 날짜를 추적할 수 있습니다.<br/><br/>클라이언트 응용 프로그램 오류 처리에는 사용자가 인증해야 함을 나타내는 [오류](/help/authentication/integration-guide-programmers/features-standard/error-reporting/enhanced-error-codes.md#enhanced-error-codes-lists-rest-api-v2) 코드(예: `authenticated_profile_missing`, `authenticated_profile_expired`, `authenticated_profile_invalidated` 등)를 처리해야 합니다. |

#### &#x200B;9. 클라이언트 애플리케이션은 재인증 없이 사용자 프로필을 확장할 수 있습니까? {#authentication-phase-faq9}
...
#### &#x200B;10. 사용 가능한 각 프로필 엔드포인트의 사용 사례는 무엇입니까? {#authentication-phase-faq10}
...
| API | 설명 | 사용 사례 |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [프로필 끝점 API](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profiles.md) | 모든 사용자 프로필을 검색합니다. | **사용자가 처음으로 클라이언트 응용 프로그램을 엽니다**<br/><br/>&#x200B;이 시나리오에서는 클라이언트 응용 프로그램에 사용자가 선택한 MVPD 식별자가 영구 저장소에 캐시되지 않습니다.<br/><br/>그 결과 사용 가능한 모든 사용자 프로필을 검색하기 위한 단일 요청이 전송됩니다. |
| [특정 MVPD API에 대한 프로필 끝점](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-mvpd.md) | 특정 MVPD과 연결된 사용자 프로필을 검색합니다. | **사용자가 이전 방문에서 인증한 후 클라이언트 응용 프로그램으로 돌아갑니다**<br/><br/>&#x200B;이 경우 클라이언트 응용 프로그램에는 이전에 선택한 사용자의 MVPD 식별자가 영구 저장소에 캐시되어 있어야 합니다.<br/><br/>그 결과 특정 MVPD에 대한 사용자의 프로필을 검색하기 위한 단일 요청이 전송됩니다. |
| [특정(인증) 코드 API에 대한 프로필 끝점](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) | 특정 인증 코드와 연관된 사용자 프로필을 검색합니다. | **사용자가 인증 프로세스를 시작합니다**<br/><br/>&#x200B;이 시나리오에서는 클라이언트 응용 프로그램이 사용자가 인증을 성공적으로 완료했는지 확인하고 프로필 정보를 검색해야 합니다.<br/><br/>그 결과 인증 코드와 연결된 사용자 프로필을 검색하기 위한 폴링 메커니즘이 시작됩니다. |
...
#### &#x200B;13. 사용자 프로필은 언제 유효하지 않게 됩니까? {#authentication-phase-faq13}
...
#### &#x200B;14. 클라이언트 애플리케이션은 언제 폴링 메커니즘을 시작해야 합니까? {#authentication-phase-faq14}
...
#### &#x200B;15. 클라이언트 애플리케이션은 언제 폴링 메커니즘을 중지해야 합니까? {#authentication-phase-faq15}
...
[세션](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/sessions-apis/rest-api-v2-sessions-apis-create-authentication-session.md) 끝점 응답에서 `notAfter` 타임스탬프(예: 30분)에 표시된 대로 인증 세션 및 코드가 만료됩니다. 이 경우 사용자는 인증 프로세스를 다시 시작해야 하며 이전 인증 코드를 사용한 폴링을 즉시 중지해야 합니다.
...
#### &#x200B;17. 클라이언트 애플리케이션이 보낼 수 있는 최대 폴링 요청 수는 얼마입니까? {#authentication-phase-faq17}
...
#### &#x200B;21. 클라이언트 애플리케이션은 크로스 디바이스 SSO(Single Sign-On) 액세스를 어떻게 관리해야 합니까? {#authentication-phase-faq21}
...
#### &#x200B;3. 사전 승인 결정은 무엇입니까? {#preauthorization-phase-faq3}
...
#### &#x200B;4. 클라이언트 애플리케이션이 영구 저장소에서 사전 인증 결정을 캐시해야 합니까? {#preauthorization-phase-faq4}
...
#### &#x200B;7. 사전 인증 결정이 이미 있는 경우 인증 단계를 건너뛸 수 있습니까? {#preauthorization-phase-faq7}
...
#### &#x200B;9. 클라이언트 애플리케이션이 한 번에 사전 권한 부여 결정을 받을 수 있는 리소스는 몇 개입니까? {#preauthorization-phase-faq9}
...
#### &#x200B;1. 인증 단계의 목적은 무엇입니까? {#authorization-phase-faq1}
...
#### &#x200B;6. 미디어 토큰은 무엇이며 유효기간은 얼마나 됩니까? {#authorization-phase-faq6}
...
#### &#x200B;9. 인증 결정에 있는 각 타임스탬프 속성의 목적은 무엇입니까? {#authorization-phase-faq9}
...
#### &#x200B;2. 로그아웃 단계는 필수입니다? {#logout-phase-faq2}
...
#### &#x200B;1. Authorization 헤더의 값을 계산하는 방법 {#headers-faq1}
...
#### &#x200B;2. OpenAPI 지원을 통해 API 개발 도구를 사용하여 REST API V2 요청 및 응답을 탐색할 수 있습니까? {#misc-faq2}
...
### APPLE SSO FAQ {#apple-sso-general}

+++APPLE SSO FAQ

#### &#x200B;1. Apple SSO란 무엇이며 REST API V2에서 어떻게 작동합니까? {#apple-sso-faq1}

Apple SSO(Single Sign-On)를 사용하면 사용자가 Apple의 [비디오 구독자 계정 프레임워크](https://developer.apple.com/documentation/videosubscriberaccount)를 사용하여 장치 시스템 수준에서 TV 공급자 계정에 로그인할 수 있으므로 앱별로 인증할 필요가 없습니다.

REST API V2는 파트너 흐름을 통해 iOS, iPadOS 또는 tvOS에서 실행되는 클라이언트 애플리케이션의 최종 사용자에게 Partner SSO(Single Sign-On)를 지원합니다.

자세한 내용은 다음 문서를 참조하십시오.

* [Apple SSO 개요](/help/authentication/integration-guide-programmers/features-standard/sso-access/partner-sso/apple-sso/apple-sso-overview.md)
...
#### &#x200B;2. Apple SSO를 구현하기 위한 사전 요구 사항은 무엇입니까? {#apple-sso-faq2}

Apple SSO를 구현하기 전에 다음 전제 조건이 충족되는지 확인하십시오.

**스트리밍 응용 프로그램 요구 사항:**

* Apple에 문의하여 [비디오 구독자 계정 프레임워크](https://developer.apple.com/documentation/videosubscriberaccount)를 Apple 팀 ID의 일부로 사용하도록 설정하십시오.
* 비디오 구독자 단일 사인온 권한 부여를 Apple 개발자 계정의 일부로 구성합니다.
* Xcode 버전 8 이상 및 iOS/tvOS 버전 10 이상을 사용합니다.
* 장치 수준에서 구독 정보에 액세스할 수 있는 사용자 권한을 요청합니다.
* Adobe Pass 인증 백엔드가 장치 플랫폼을 식별할 수 있도록 `X-Device-Info` 및/또는 `User-Agent` 헤더를 포함합니다.
* 모든 관련 API 요청에 유효한 파트너 프레임워크 상태의 `AP-Partner-Framework-Status` 헤더를 포함하십시오.

**Adobe Pass 구성:**

* `Enable Single Sign On` 속성을 `Yes`(으)로 설정하여 Adobe Pass TVE 대시보드를 통해 원하는 각 통합 및 플랫폼(iOS/tvOS)에 대해 SSO(Single Sign-On)를 사용하도록 설정합니다.

**MVPD 요구 사항:**

* Apple SSO 지원을 위해 MVPD을 Apple과 온보딩해야 합니다.
* MVPD은 파트너 SSO 구성을 위해 Adobe Pass 인증과 함께 온보딩되어야 합니다.

자세한 내용은 [Apple SSO 개요 - 사전 요구 사항](/help/authentication/integration-guide-programmers/features-standard/sso-access/partner-sso/apple-sso/apple-sso-overview.md#apple-sso-prerequisites) 설명서를 참조하십시오.

#### &#x200B;3. AP-Partner-Framework-Status 헤더는 무엇이며 헤더가 필요한 이유는 무엇입니까? {#apple-sso-faq3}

`AP-Partner-Framework-Status` 헤더에는 Apple의 비디오 구독자 계정 프레임워크에서 검색한 파트너 프레임워크 상태 정보가 포함되어 있습니다.
...
스트리밍 애플리케이션은 비디오 구독자 계정 프레임워크를 호출하여 이 정보를 검색하고 이를 헤더에 base64 인코딩 JSON 페이로드로 포함해야 합니다.

**모범 사례:**

* 스트리밍 애플리케이션은 애플리케이션이 포그라운드 상태에 들어갈 때 파트너 프레임워크 상태를 검색해야 합니다.
* 파트너 프레임워크 상태 정보를 캐시하고 애플리케이션이 배경에서 전경으로 전환할 때 새로 고칩니다.
* 파트너 프레임워크 상태에 유효한 값(사용자 권한 부여, 유효한 공급자 식별자, 유효한 만료 날짜)이 포함되어 있는지 확인하십시오.

자세한 내용은 [AP-Partner-Framework-Status](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/appendix/headers/rest-api-v2-appendix-headers-ap-partner-framework-status.md) 설명서를 참조하십시오.

#### &#x200B;4. Apple SSO 문제를 해결하려면 어떻게 합니까? {#apple-sso-faq4}

Apple SSO 문제를 해결할 때 다음과 같은 일반적인 방법을 사용하십시오.

**1단계: SAML 요청 생성 확인**

* [파트너 인증 요청 검색](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/partner-single-sign-on-apis/rest-api-v2-partner-single-sign-on-apis-retrieve-partner-authentication-request.md) 끝점이 올바른 파트너 인증 요청(SAML 요청)을 반환하는지 확인하십시오.
* base64 디코딩 후 `authenticationRequest - request` 특성에 올바른 형식의 SAML 요청이 포함되어 있는지 확인하십시오.
* `AP-Partner-Framework-Status` 헤더에 올바른 파트너 프레임워크 상태 정보가 포함되어 있는지 확인하십시오.

**2단계: VSA 오류 식별**

* 비디오 구독자 계정 프레임워크에서 오류 응답을 검토합니다.
* 오류 코드 및 설명은 [비디오 구독자 계정 프레임워크](https://developer.apple.com/documentation/videosubscriberaccount/vserror#Error-codes) 설명서를 참조하십시오.
* VSA 오류 설명서는 매우 일반적이며 자세한 근본 원인 정보를 제공하지 않을 수 있습니다.

**3단계: 일반적인 문제 확인**

* 사용자 권한 액세스 상태를 부여해야 합니다(`VSAccountAccessStatus.granted` 확인).
* 사용자 공급자 매핑 식별자가 있어야 하며 올바른(`accountProviderIdentifier`)이어야 합니다.
* 사용자 공급자 프로필의 만료 날짜는 올바른(`authenticationExpirationDate`)이어야 합니다.
* MVPD은 Apple과 온보딩해야 합니다(구성 끝점 응답에서 `boardingStatus` 확인).
* MVPD 통합에는 TVE 대시보드에서 Apple SSO가 활성화되어 있어야 합니다.

**4단계: 조사를 위해 MVPD 참여**

* 비디오 구독자 계정 프레임워크가 유효한 SAML 요청을 받았지만 MVPD 상호 작용 후 SAML 응답을 반환하지 않는 경우 Apple SSO를 사용할 수 있는 MVPD에서 문제를 해결하도록 하십시오.
* 이 문제는 Apple 측의 MVPD 특정 구성 또는 구현과 관련이 있을 수도 있습니다.

#### &#x200B;5. 일반적인 VSA 오류는 무엇이며 어떻게 처리해야 합니까? {#apple-sso-faq5}

일반적인 시나리오 및 처리:

**사용자가 권한 거부:**

* `VSAccountAccessStatus`은(는) `.granted`이(가) 아닙니다.
* 기본 인증 흐름으로 돌아가서 애플리케이션 고유의 MVPD 선택기를 표시합니다.

**MVPD이 Apple과 함께 온보딩되지 않음(오류 코드 1):**

* VSA 프레임워크에서 `error.code == 1` 오류를 반환합니다.
* `error.userInfo["VSErrorInfoKeyUnsupportedProviderIdentifier"]`에 Apple MSO ID가 포함되어 있습니다.
* 기본 인증 흐름으로 돌아가지만, 구성의 MVPD에 Apple MSO ID를 매핑할 수 있는 경우 MVPD 선택기로 사용자에게 묻는 메시지를 건너뛸 수 있습니다.

**반환된 메타데이터가 없습니다.**

* `vsaMetadata`이(가) `nil`이거나 필수 필드가 누락되었습니다.
* 기본 인증 흐름으로 돌아갑니다.

**SAML 응답이 반환되지 않음:**

* MVPD 인증 후 `samlAttributeQueryResponse`이(가) `nil`입니다.
* MVPD의 Apple SSO 구현에 문제가 있을 수 있습니다.
* 조사를 위해 Adobe, MVPD 및 Apple에 연락하는 것이 좋습니다.

자세한 오류 정보는 [비디오 구독자 계정 프레임워크](https://developer.apple.com/documentation/videosubscriberaccount) 설명서를 참조하십시오.

#### &#x200B;6. 프로필 유형 &quot;appleSSO&quot;는 무엇을 나타냅니까? {#apple-sso-faq6}

`type`이(가) &quot;appleSSO&quot;로 설정된 프로필은 사용자가 비디오 구독자 계정 프레임워크를 사용하여 Apple의 파트너 SSO(Single Sign-On) 흐름을 통해 인증했음을 나타냅니다.

이 프로필 유형에는 다음과 같은 특정 요구 사항이 있습니다.

* &quot;appleSSO&quot; 프로필을 사용하여 결정(사전 인증/인증) 요청을 할 때 스트리밍 애플리케이션에는 현재 파트너 프레임워크 상태의 올바른 `AP-Partner-Framework-Status` 헤더가 포함되어야 합니다.
* 로그아웃 중에 응답에는 사용자가 파트너(시스템) 수준에서 로그아웃을 완료해야 함을 나타내는 &quot;partner_logout&quot;으로 설정된 `actionName`과(와) &quot;partner_interactive&quot;로 설정된 `actionType`이(가) 포함됩니다.

일반 프로필(Apple SSO 이외)에는 이러한 요구 사항이 없으며 기본 인증 흐름을 따릅니다.

#### &#x200B;7. Apple SSO 프로필에 대한 로그아웃을 처리하려면 어떻게 해야 합니까? {#apple-sso-faq7}
...
자세한 내용은 [Apple SSO Cookbook(REST API V2) - 로그아웃 단계](/help/authentication/integration-guide-programmers/features-standard/sso-access/partner-sso/apple-sso/apple-sso-cookbook-rest-api-v2.md#cookbook) 설명서를 참조하십시오.

#### &#x200B;8. Apple SSO가 실패할 경우 기본 인증으로 돌아갈 수 있습니까? {#apple-sso-faq8}

예. Adobe Pass 인증 REST API V2는 다음 시나리오에서 자동으로 기본 인증 흐름으로 돌아갑니다.

**자동 대체:**

* Adobe Pass 백엔드에서 파트너 SSO(Single Sign-On) 유효성 검사에 실패했습니다
* 사용자가 구독 정보에 대한 액세스 권한을 거부합니다.
* MVPD은 Apple과 온보딩되지 않습니다.
* VSA 프레임워크가 유효한 메타데이터를 반환하지 않음

**응답 표시:**

기본 인증으로 되돌리면 [파트너 인증 요청 검색](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/partner-single-sign-on-apis/rest-api-v2-partner-single-sign-on-apis-retrieve-partner-authentication-request.md) 끝점 응답에 다음이 포함됩니다.

* `actionName`이(가) &quot;인증&quot; 또는 &quot;다시 시작&quot;으로 설정됨
* `actionType`이(가) &quot;대화형&quot; 또는 &quot;직접&quot;으로 설정됨

스트리밍 애플리케이션은 기본 인증 플로우를 시작하여 이러한 응답을 처리해야 합니다.

**수동 대체 항목:**

특정 통합에 대해 Apple SSO를 비활성화하고 항상 기본 인증을 사용하려면 다음 작업을 수행하십시오.

* 원하는 통합 및 플랫폼(iOS/tvOS)에 대해 Adobe Pass TVE 대시보드에서 `Enable Single Sign On` 속성을 `No`(으)로 설정합니다.

자세한 내용은 [파트너 흐름을 사용하여 Single Sign-On](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/flows/single-sign-on-access-flows/rest-api-v2-single-sign-on-partner-flows.md) 설명서를 참조하세요.

#### &#x200B;9. 비디오 구독자 계정 프레임워크에 대한 자세한 내용은 어디에서 찾을 수 있습니까? {#apple-sso-faq9}

API 참조, 오류 코드 및 통합 지침을 포함하여 Apple의 비디오 구독자 계정 프레임워크에 대한 자세한 내용은 공식 Apple 설명서를 참조하십시오.

* [비디오 구독자 계정 프레임워크 설명서](https://developer.apple.com/documentation/videosubscriberaccount)

검토할 주요 클래스 및 프로토콜:

* [VSAccountManager](https://developer.apple.com/documentation/videosubscriberaccount/vsaccountmanager) - 구독자 계정 작업을 위한 기본 관리자
* [VSAccountMetadataRequest](https://developer.apple.com/documentation/videosubscriberaccount/vsaccountmetadatarequest) - 구독자 계정 정보 요청
* [VSAccountMetadata](https://developer.apple.com/documentation/videosubscriberaccount/vsaccountmetadata) - 구독자 계정 정보가 포함된 응답
* [VSAccountManagerDelegate](https://developer.apple.com/documentation/videosubscriberaccount/vsaccountmanagerdelegate) - 계정 관리자 이벤트에 대한 위임 프로토콜
* [VSAccountAccessStatus](https://developer.apple.com/documentation/videosubscriberaccount/vsaccountaccessstatus) - 사용자 권한 상태 열거형
...
#### &#x200B;4. 향상된 오류 코드는 REST API V2에서 기본적으로 활성화되어 있습니까? {#migration-faq4}
