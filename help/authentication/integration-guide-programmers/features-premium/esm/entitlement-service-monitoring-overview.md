...
TVE 사이트 및 앱은 연중무휴 24시간 사용 가능해야 하므로 고객은 문제를 가능한 한 빨리 감지하고 수정하기 위해 자격 부여 이벤트에 실시간 insight을 사용해야 합니다. 또한 대량의 트래픽을 제공하는 플랫폼과 구현이 잘못되고 전환율이 낮은 플랫폼을 파악하기 위해 월별 데이터를 분석해야 합니다.
...
| 고유 세션 | 선택한 시간 간격 내에 Adobe Pass 인증 서비스에 대한 인증 흐름 호출을 수행한 고유 세션 수입니다. (이 지표는 일일 값을 요청한 경우에만 표시됩니다.) </br> 이는 각 개별 데이터 센터에 대해 계산됩니다. &quot;dc&quot; 차원이 요청되지 않으면 이 지표가 표시되지 않습니다. |
...
| cdt | 현재 Clientless에 사용되는 디바이스 플랫폼(대체 요소)입니다. </br>  값은 다음과 같습니다. </br> - N/A - 이벤트가 Clientless SDK에서 발생하지 않음 </br> - 알 수 없음 - Clientless API의 deviceType 매개 변수는 선택 사항이므로 값을 포함하지 않는 호출이 있습니다. </br> - Clientless API를 통해 전송된 다른 값(예: xbox, appletv, roku 등) </br> |
