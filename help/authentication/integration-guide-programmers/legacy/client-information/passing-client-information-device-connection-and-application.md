...
| #mandatory | osName | # 예 | 장치의 운영 체제(OS) 이름입니다. | # 값이 제한됨: Android Chrome OS Linux Mac OS OS X OpenBSD Roku OS Windows iOS tvOS webOS |
|            | osFamily | 예 | 장치의 운영 체제(OS) 그룹 이름입니다. | # 값이 제한됩니다. Android BSD 리눅스 PlayStation OS Roku OS Symbian Tizen Windows iOS macOS tvOS webOS |
...
|            | browserName | # 예 | 브라우저의 이름입니다. | # 값이 제한됨: Android 브라우저 Chrome Edge Firefox Internet Explorer Opera Safari SeaMonkey Symbian 브라우저 |
...
|            | browserVersion | 아니요 | 장치의 브라우저 버전입니다. | e.g. 60.0.3112 |
...
|            | displayPi | 아니요 | 장치의 실제 화면 픽셀 밀도입니다. | e.g. 294 |
|            | 대각 화면 크기 | 아니요 | 디바이스의 물리적 화면 대각선 치수(인치)입니다. | e.g. 5.5, 10.1 |
|            | connectionIp | 아니요 | HTTP 요청을 보내는 데 사용되는 장치의 IP입니다. | e.g. 8.8.4.4 |
|            | connectionPort | 아니요 | HTTP 요청을 전송하는 데 사용되는 장치의 포트입니다. | e.g. 53124 |
...
|   | 공급업체 | Build.BRAND | 삼성 |
|   | 제조업체 | Build.MANUFACTURING | 삼성 |
| ! | 버전 | Build.DEVICE | jflte |
|   | displayWidth | DisplayMetrics.widthPixel | 600 |
...
|   | 제조업체 | Build.MANUFACTURING | Amazon |
| ! | 버전 | Build.DEVICE | 몬토야 |
|   | displayWidth | DisplayMetrics.widthPixel |              |
...
|   | displayWidth | 디스플레이 정보.화면 너비InRawPixels | 1920 |
|   | displayHeight | 디스플레이 정보.화면 높이InRawPixels | 1080 |
