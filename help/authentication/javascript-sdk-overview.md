...
일반 Adobe Pass 인증 권한 흐름은 [프로그래머 권한 흐름](/help/authentication/entitlement-flow.md)에 포함되어 있으며 JavaScript 통합 Cookbook은 구현을 안내합니다. 다음 섹션에서는 JavaScript AccessEnabler 통합에 대한 설명과 샘플을 제공합니다.
...
고객의 공급자가 누구인지 이미 알고 있는 경우 사용자 상호 작용 없이 [프로그래밍 방식으로 MVPD를 설정](/help/authentication/home.md)할 수 있습니다. 이 기술은 동일하지만 공급자 선택기 대화 상자를 호출하여 고객에게 MVPD를 선택하라고 요청하는 단계를 거치지 않습니다.
...
- **Adobe Pass 인증과 통합되지 않은 사이트에서 로그아웃이 시작되면** 이 경우 MVPD는 브라우저 리디렉션을 통해 Adobe Pass 인증 단일 로그아웃 서비스를 호출할 수 있습니다. (백채널 호출을 통한 SLO 호출은 현재 지원되지 않습니다.)
