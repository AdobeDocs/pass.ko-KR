...
예기치 않은 오류가 발생하는 경우(예: 네트워크 문제, MVPD 인증 끝점을 사용할 수 없음 등) Adobe Pass 인증 서비스에서 사전 승인 API 요청을 처리할 때 발생하는 하나 이상의 분리된 오류 정보는 사전 승인 API 응답 결과의 일부로 영향을 받는 리소스에 포함됩니다.
