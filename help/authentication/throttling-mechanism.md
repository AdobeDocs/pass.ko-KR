...
제안된 조절 메커니즘은 &quot;X-Forwarded-For&quot; 헤더를 통해 식별된 디바이스를 개별적으로 사용합니다.
각 장치에 대해 동일한 방식으로 제한이 적용됩니다.
...
현재, 기본 제한은 초당 최대 1개의 요청을 허용하며, 초기 버스트는 3개입니다(식별된 클라이언트의 첫 번째 상호 작용에 대해 1회 허용, 초기화를 성공적으로 완료할 수 있음).
이는 모든 고객의 일반적인 비즈니스 사례에 영향을 미치지 않습니다.
...
| 두 번째 1.4 | 호출이 429 상태 코드를 수신함 | 제한에서 2개의 호출이 소비되고, 버스트로 표시된 3개의 호출과 &#39;429개의 요청이 너무 많음&#39;을 받습니다. |
| 두 번째 1.6 | 호출이 429 상태 코드를 수신함 | 제한에서 2개의 호출이 소비되고 3개의 호출이 버스트로 표시되며 2개의 호출에는 &#39;429개의 요청이 너무 많음&#39;이 수신됩니다. |
| 두 번째 1.8 | 호출이 429 상태 코드를 수신함 | 제한에서 2개의 호출이 소비되고, 버스트로 표시된 3개의 호출과 &#39;429개의 요청이 너무 많음&#39;을 수신함 |
| 두 번째 2.1 | 호출이 성공 상태 코드를 수신함 | 제한에서 3개의 호출이 소비되고, 버스트로 표시된 3개의 호출과 &#39;429개의 요청이 너무 많음&#39;을 수신함 |
