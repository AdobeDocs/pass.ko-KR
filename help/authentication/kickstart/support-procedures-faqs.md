...
* 인증 프로세스 중에 사용자가 인증 흐름을 다시 시작하지 못하고 Adobe 오류 페이지에서 멈춥니다.
...
Adobe은 이러한 문제에 대한 조사를 지원하지만 심각도 1 수준 사건으로 간주되지 않습니다.
...
1. Adobe은 이 문제를 해결하기 위해 노력하고 있으며 영향을 받는 모든 파트너에게 계속 정보를 제공합니다.

파트너 문제(프로그래머/MVPD)의 경우:

1. Adobe은 MVPD 또는 프로그래머 사이트 중 하나와의 통합과 관련된 문제를 식별합니다.

1. Adobe은 해당 파트너와 적절한 지원 절차에 따라 영향을 받는 파트너에게 통지하고 파트너의 지원 조직과 티켓을 엽니다.

1. 영향 분석 중에 Adobe에서 문제가 사고 시나리오에 대해 미리 합의된 결정 중 하나에 해당하는 것으로 식별되면 파트너의 입력을 기다리지 않고 적절하게 작동합니다.
...
|    | 시나리오 | 설명 | 액션 |
|----|--------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| S1 | Adobe은 정상 프로덕션 작업 중 MVPD의 통합 문제를 식별합니다. | 일반적인 프로덕션 작업 중에 Adobe은 인증/권한 부여 흐름(예: 만료된 인증서, 만료된 SAML 응답, 닫힌 포트, 변경된 매개 변수 등)을 수행할 수 없는 MVPD 중 하나의 문제를 확인합니다. | Adobe은 영향을 받는 MVPD 및 프로그래머에게 알립니다.  </br></br> Adobe은 영향을 받는 모든 프로그래머에 대해 이 MVPD을 비활성화합니다. </br></br> Adobe에서 해당 MVPD과의 합의된 지원 절차에 따라 MVPD으로 티켓을 엽니다. |
| S2 | Adobe은 프로그래머를 위한 새 MVPD을 활성화하고 프로그래머는 시작 날짜 이전에 MVPD을 허용합니다. | Adobe이 프로그래머 사이트에 대해 새 MVPD을 활성화하고 있고 사이트에서는 그렇게 하지 않았더라도 선택기에 새 MVPD을 이미 표시하고 있습니다. | Adobe은 예약된 날짜 이전에 선택기에 표시되는 새 MVPD에 대해 프로그래머에게 알립니다. </br></br> 프로그래머는 필요한 경우 선택기에서 제거하기 위한 작업을 수행합니다. |
| S3 | Adobe은 MVPD이 프로덕션에 들어갈 준비가 되지 않았더라도 프로그래머를 위해 새 MVPD을 활성화합니다 | Adobe은 프로그래머를 위한 새 MVPD을 활성화하고 있지만 MVPD은 아직 통합 지원을 배포하지 않았기 때문에 인증/권한 부여 흐름을 수행할 수 없습니다 | 프로그래머 </br></br>이(가) 요청한 경우에만 Adobe에서 배포를 수행합니다. 모든 테스트가 수행되면 프로그래머가 MVPD의 허가를 확인해야 합니다. |
