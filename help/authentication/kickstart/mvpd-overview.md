...
인증이란? 인증은 MVPD에서 지정된 사용자가 알려진 고객임을 확인하는 프로세스입니다.



인증이란 무엇입니까? 인증은 인증된 사용자가 지정된 리소스에 대해 유효한 구독을 가지고 있음을 MVPD이 확인하는 프로세스입니다.
...
>[!NOTE]
>
>MVPD의 경우 웹 서비스는 Adobe Pass 인증의 인증 및 권한 부여 요청을 인식하고 필요한 데이터로 예상 형식으로 응답할 수 있어야 합니다.
>

Adobe Pass 인증을 사용하면 고객에게 SSO(Single Sign-On) 인증 및 인증이라고도 하는 통합 ID 관리를 제공할 수 있습니다. Adobe Pass 인증을 사용하면 MVPD에서 해당 인증을 지속할 수 있는 한 구독자가 첫 번째 인증 후 다시 로그인할 필요가 없습니다. (일반적으로 30일) 이를 위해 Adobe Pass 인증은 고객을 위한 인증 토큰에 대한 공통 도메인을 제공합니다. 이 인증 상태 정보는 주어진 MVPD과 통합된 모든 참여 사이트에서 사용할 수 있습니다.
...
Adobe은 MVPD 기술 팀과 협력하여 기존 통합 요구 사항을 충족하도록 Adobe Pass 인증을 구성합니다. &quot;표준&quot; 통합과 최소한의 지원 요구 사항(설명서 및 기본 이메일 지원)을 전제로 하는 경우 MVPD에 대한 통합은 무료입니다. MVPD에 중요한 지원 또는 에스컬레이션된 타임라인이 필요한 경우 지원 수수료가 청구되거나 제공업체는 Synacor와 같은 당사의 솔루션에 익숙한 제3자와 협력할 수 있습니다.


Adobe Pass 인증은 다음과 같이 MVPD 비즈니스 논리를 효율적으로 처리할 수 있도록 지원합니다.

자체 포함된 비즈니스 로직의 경우 승인 요청이 수신되면 MVPD에서 적용할 수 있으며 MVPD이 승인 요청을 수신하면 Adobe에서 비즈니스 로직 적용을 지원하는 데 필요한 데이터를 제공합니다. 이 데이터에는 요청을 하는 사용자의 고유 장치 ID 및 장치의 IP 주소가 포함될 수 있지만 이에 제한되지 않습니다.

사용자 개입 및/또는 Adobe 솔루션에 의한 특정 처리가 필요한 비즈니스 논리의 경우 Adobe은 각 MVPD에 대해 일부 사용자 지정 속성을 유지 관리할 수 있습니다. 이러한 MVPD 전용 구성/정책에는 최상위 워크플로의 특정 지점에서 시작할 수 있는 사전 정의된 워크플로가 포함되어 있습니다. 사용자 지정 속성 지원에 대한 자세한 내용은 Adobe 담당자에게 문의하십시오.

다음 다이어그램은 MVPD 및 프로그래머와 이러한 Adobe Pass 인증 구성 요소의 관계를 보여 줍니다.
...
Access Enabler는 사용자와의 모든 인증 및 권한 부여를 용이하게 하며 시스템에서 로컬로 실행됩니다. 프로그래머는 상위 수준 웹 페이지나 플레이어 애플리케이션에 대한 책임을 유지하는 반면 MVPD을 사용하여 실제 권한 부여 워크플로를 처리하는 액세스 Enabler입니다.

클라이언트 없는 웹 서비스는 웹 페이지를 렌더링할 수 없는 장치에 대해 Adobe Pass 인증에서 제공합니다.  이러한 장치의 경우 권한 부여 프로세스가 시작되고 스마트 장치에서 컨텐츠를 보는 반면 MVPD을 통한 인증은 웹 지원 장치(PC, 스마트폰 및 태블릿)에서 발생합니다.

Access Enabler:

* MVPD 관련 인증 및 권한 부여 워크플로우를 시작합니다.
* 프로그래머 리소스/채널당 성공적인 인증 응답을 캐시하여 불필요한 요청 트래픽을 최소화합니다.
* 명시적 장치 등록과 같이, 각 MVPD과 관련된 사전 정의된 워크플로우에 대해 구성할 수 있습니다.
* 다음 양식으로 사용할 수 있습니다.
...
**오래 지속된 인증 토큰**. 고객이 Adobe Pass 인증을 사용하여 MVPD 계정에 성공적으로 로그인하면 인증이 성공합니다. 그런 다음 Adobe Pass 인증은 요청 장치에 연결된 장기 인증(&quot;authN&quot;) 토큰과 (MVPD에 따라) 사용자를 익명으로 식별하는 글로벌 고유 식별자(&quot;GUID&quot;)를 생성합니다.
...
다음 단계에서는 Adobe Pass 인증 인증 흐름의 예를 보여 줍니다.  이것은 프로그래머가 사용자가 MVPD의 유효한 고객인지 확인하는 자격 부여 프로세스의 일부입니다.  이 시나리오에서는 사용자가 MVPD의 올바른 구독자입니다.  사용자가 프로그래머 Flash 응용 프로그램을 사용하여 보호된 콘텐츠를 보려고 합니다.

1. 사용자가 프로그래머 Flash 응용 프로그램과 Adobe Pass Authentication Access Enabler 구성 요소를 사용자 컴퓨터에 로드하는 프로그래머 웹 페이지로 이동합니다. Flash 애플리케이션은 Access Enabler를 사용하여 프로그래머의 ID를 Adobe Pass 인증으로 설정하고 Adobe Pass 인증은 Access Enabler를 해당 프로그래머(&quot;요청자&quot;)에 대한 구성 및 상태 데이터로 채웁니다. Access Enabler는 다른 API 호출을 수행하기 전에 서버에서 이 데이터를 받아야 합니다.  기술 참고: 프로그래머는 Access Enabler의 `setRequestor()` 메서드를 사용하여 ID를 설정합니다.
1. 사용자가 프로그래머로 보호된 콘텐츠를 보려고 하면 프로그래머 애플리케이션은 MVPD의 목록을 사용자에게 제공하고, 사용자는 이 목록에서 공급자를 선택합니다.
1. 사용자는 Adobe Pass 인증 서버로 리디렉션되며, 여기서 사용자가 선택한 MVPD에 대한 암호화된 SAML 요청을 만듭니다. 이 요청은 프로그래머를 대신하여 MVPD에 인증 요청으로 전송됩니다. 그런 다음 MVPD 시스템에 따라 사용자의 브라우저가 MVPD 사이트로 리디렉션되어 로그인되거나 프로그래머 앱에서 로그인 iFrame이 만들어집니다.
1. 두 경우(리디렉션 또는 iFrame) 중 하나에서 MVPD은 요청을 수락하고 해당 로그인 페이지를 표시합니다.
1. 사용자가 MVPD에 로그인하고 MVPD이 결제 고객으로서 사용자 상태를 확인한 다음, MVPD이 자체 HTTP 세션을 생성합니다.
1. 사용자의 유효성을 검사하면 MVPD에서 응답(SAML 및 암호화됨)을 만들어 MVPD에서 이 응답을 Adobe Pass 인증으로 다시 보냅니다.
...
1. 보호된 콘텐츠의 특정 항목은 &quot;리소스 식별자&quot;로 표시됩니다.  이는 간단한 문자열 또는 더 복잡한 구조일 수 있지만 어떤 경우든 리소스 식별자의 특성은 프로그래머와 MVPD 간에 미리 합의됩니다.  프로그래머 애플리케이션은 리소스 식별자를 Access Enabler에 전달합니다.  Access Enabler는 사용자의 로컬 시스템 또는 디바이스에서 AuthZ 토큰을 확인합니다.  AuthZ 토큰이 없으면 Access Enabler가 요청을 백엔드 Adobe Pass 인증 서버에 전달합니다.
