...
이 경우 중복 제거는 다른 MVPD 사용자가 동일한 사용자 ID를 받지 않아야 하므로 영향을 주지 않습니다. 서로 다른 두 브랜드의 합계를 만들 때 동일한 MVPD을 사용하면 중복 제거 효과가 훨씬 커집니다.
...
이 작업은 `getAuthorization()`을(를) 호출할 때 자동으로 트리거되며 Adobe Pass 인증을 사용한 검사만 포함합니다. MVPD은 이 흐름에 포함되지 않습니다.
...
위의 다이어그램에 표시된 것처럼 인증 이벤트는 사용자가 MVPD을 왕복할 때만 트리거되며 인증 이벤트는 캐시된 토큰 인증을 포함하지 않습니다.

인증 시도 이벤트는 사용자가 선택기에서 특정 MVPD을 클릭한 후 트리거됩니다.

* MVPD 측에서 이에 가까운 첫 번째 이벤트는 페이지 로드입니다
* Adobe Pass 인증은 사용자가 MVPD 페이지에 로그인하기 위해 반복적으로 시도하는 것을 카운트하지 않습니다(잘못된 암호, 다시 시도)
...
사용자는 일반적으로 유료 TV 가입이지만 때로는 인터넷 액세스만 있는 MVPD의 알려진 가입자입니다. 사용자가 MVPD에 유효한 자격 증명을 명시적으로 입력했거나, 이전에 유효한 자격 증명을 입력하고 &quot;내 정보 저장&quot;을 체크했기 때문에(그리고 이전 세션이 만료되지 않았기 때문에) 성공적인 인증이 발생할 수 있습니다.

따라서 MVPD에서 Adobe Pass 인증을 인증 요청에 대한 양의 응답으로 보내고 Adobe Pass 인증에서 *AuthN 토큰*&#x200B;을 만듭니다.
...
일부 인증의 경우 권한 부여와 동일시되므로 일부 MVPD가 별도의 권한 부여 단계를 지원하는 것은 아닙니다. MVPD이 Adobe Pass 인증을 백채널 AuthZ 요청에 대한 성공적인 응답으로 보내고 Adobe Pass 인증이 AuthZ 토큰을 만듭니다.
...
승인이 거부되면 인증된 사용자는 요청된 프로그래밍에 대해 확정된 구독을 보유하지 않습니다. 가장 가능성 있는 원인은 채널이 사용자의 구독 패키지에 포함되지 않기 때문이지만, 이는 MVPD에서 인터넷 액세스만 하는 사용자를 반영할 수도 있습니다.

일부 MVPD의 경우, 사용자는 MVPD의 인터넷 구독만 있어도(유료 TV 구독 없음) 성공적으로 인증됩니다. 이 경우 사용자가 권한 부여를 요청하는 채널이 기본 패키지에 있더라도 권한 부여가 거부됩니다.
...
* AuthN 및 AuthZ 토큰은 일정 기간 동안 MVPD 응답을 캐시하므로 성공적인 재생 요청 이벤트는 지표에서 가장 빈번한 이벤트입니다.
...
이 값은 장치 간에 지속적이거나(MVPD은 로그인 위치에 관계없이 주어진 사용자에 대해 동일한 값을 생성함) 각 로그인에 대해 MVPD이 백 엔드에 매핑하는 새 값이 생성될 수 있습니다. 일반적으로 MVPD에서 Adobe Pass 인증에 제공하는 값은 세션 및 장치 간에 지속되지만, 언급된 바와 같이 지속성이 보장되거나 검증되지 않습니다.

이 값은 고유 사용자를 계산하는 방법으로 사용됩니다. (요청자 ID/간격/MVPD에 따라) 보고된 값이 특정 간격에 대해 중복 제거됩니다. 따라서 일일 고유 사용자의 합계는 일반적으로 월별 값과 다르며 월별 값은 더 낮은 값을 갖습니다.
