...
>
> * [클라이언트 자격 증명 검색](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-client-credentials.md) API 설명서에 설명된 대로 클라이언트 자격 증명을 가져옵니다.
...
사용 가능한 URL 경로는 응답에 포함된 링크를 통해 검색할 수 있습니다. 유효한 URL 경로는 집계된 지표를 보유하는 기본 드릴다운 트리 내의 경로를 매핑하기 위해 유지됩니다. `/dimension1/dimension2/dimension3` 형식의 경로는 이러한 세 차원의 사전 집계를 반영합니다(`clause GROUP`, `dimension1`, `dimension2`의 SQL `dimension3`에 해당). 이러한 사전 집계가 존재하지 않고 시스템이 즉시 계산할 수 없는 경우, API는 404 Not Found 응답을 반환합니다.
...
### 프로그래머가 사용할 수 있는 차원 {#progr-dimensions}
...
### MVPD에 사용할 수 있는 차원 {#mvpd-dimensions}

![](/help/authentication/assets/esm-mvpd-dimensions.png)

`https://mgmt.auth.adobe.com/esm/v3` API 끝점에 대한 GET에서 다음을 포함하는 표현을 반환합니다.
...
| 405 | 메서드가 허용되지 않음 | 지원되지 않는 메서드가 요청에 사용되었음을 나타냅니다. 현재 GET 메서드만 지원되지만, 이후 버전에서는 HEAD 또는 OPTIONS을 허용할 수 있습니다 |
